...
export default function Inventory({ 
  ...
  const [partnerForm, setPartnerForm] = useState({
    code: '', name: '', type: '매출처', bizNum: '', owner: '', email: '', phone: ''
  });
  ...
  const handlePartnerSubmit = (e) => {
    ...
    if (!partnerForm.code || !partnerForm.name) {
      alert('업체 코드와 업체명은 필수 입력 사항입니다.');
      return;
    }

    if (editingPartner) {
      setPartners(prev => prev.map(p => p.code === editingPartner.code ? { ...partnerForm } : p));
      logActivity('재고', `업체 정보 수정: ${partnerForm.name} (${partnerForm.code})`);
      setEditingPartner(null);
    } else {
      if (partners.some(p => p.code === partnerForm.code)) {
        alert('이미 존재하는 업체 코드입니다.');
        return;
      }
      setPartners(prev => [...prev, { ...partnerForm }]);
      logActivity('재고', `신규 업체 등록: ${partnerForm.name} (${partnerForm.code})`);
    }
  ...
  const filteredPartners = partners.filter(p => 
    ...
    p.bizNum.includes(partnerSearchQuery) ||
    p.owner.includes(partnerSearchQuery)
  );
    ...
    <div className="content-area">
                ...
                <table className="erp-table">
                  <thead>
                    <tr>
                      ...
                      <th>규격</th>
                      <th>단위</th>
                      <th style={{ textAlign: 'right' }}>표준매입가</th>
                      <th style={{ textAlign: 'right' }}>표준매출가</th>
                      ...
                      filteredItems.map(item => {
                        ...
                        return (
                          <tr key={item.code}>
                            ...
                            <td>{item.spec || '-'}</td>
                            <td>{item.unit}</td>
                            <td style={{ textAlign: 'right' }}>{Number(item.purchasePrice).toLocaleString()}원</td>
                            <td style={{ textAlign: 'right' }}>{Number(item.salesPrice).toLocaleString()}원</td>
                            <td style={{ textAlign: 'right' }}>{Number(item.safetyStock).toLocaleString()}</td>
                            <td style={{ textAlign: 'right', fontWeight: '700', color: isLowStock ? '#be123c' : 'inherit' }}>
                              {Number(item.stock).toLocaleString()}
                            </td>
                            <td style={{ textAlign: 'center' }}>
                              {isLowStock ? (
                                <span className="badge badge-pink">재고부족</span>
                              ) : (
                                <span className="badge badge-green">적정</span>
                              )}
                            </td>
                            <td style={{ textAlign: 'center' }}>
                    ...
                    {filteredPartners.length === 0 ? (
                      ...
                      filteredPartners.map(p => (
                        <tr key={p.code}>
                          ...
                          <td>
                            <span className={`badge ${p.type === '매출처' ? 'badge-blue' : 'badge-pink'}`}>
                              {p.type}
                            </span>
                          </td>
        ...
        <div className="panel-card">
          <div className="panel-header">
            ...
            <button 
              className="btn btn-primary"
              onClick={() => {
                const clients = partners.filter(p => p.type === '매출처');
                const defaultClientName = clients.length > 0 ? clients[0].name : '';
                const defaultClient = clients.length > 0 ? clients[0] : null;

                setSalesForm(prev => ({
                  ...prev,
                  customer: defaultClientName,
                  partnerCode: defaultClient ? (defaultClient.bizNum || defaultClient.code) : '',
        ...
        <div className="panel-card">
          <div className="panel-header">
            ...
            <button 
              className="btn btn-primary"
              onClick={() => {
                const vendors = partners.filter(p => p.type === '매입처');
                setPurchaseForm(prev => ({
                  ...prev,
      ...
      {showPartnerModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h2 className="panel-title">{editingPartner ? '업체 정보 수정' : '신규 거래처 업체등록'}</h2>
            ...
            <form onSubmit={handlePartnerSubmit}>
              <div className="modal-body">
                <div className="form-grid">
                  <div className="form-group">
                    <label className="form-label">업체 코드 *</label>
                    <input 
                      type="text" 
                      className="form-control" 
                      required
                  ...
                  <div className="form-group">
                    ...
                    >
                      <option value="매출처">매출처 (수입원/바이어)</option>
                      <option value="매입처">매입처 (원자재/제작처)</option>
                    </select>
                  </div>
                </div>

                <div className="form-group" style={{ marginBottom: '16px' }}>
                  <label className="form-label">업체명 (상호) *</label>
                  <input 
                    type="text" 
                    className="form-control" 
                    required
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <input 
                      type="text" 
                      className="form-control" 
                      value={partnerForm.bizNum}
                      onChange={(e) => setPartnerForm(prev => ({ ...prev, bizNum: e.target.value }))}
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    >
                      ...
                      {partners.filter(p => p.type === '매출처').map(p => (
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    ...
                    >
                      ...
                      {partners.filter(p => p.type === '매입처').map(p => (
                        <option key={p.code} value={p.name}>
                          {p.name}
                        </option>
                      ))}
